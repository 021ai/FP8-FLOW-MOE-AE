...
elif [ $MODEL_SIZE = qwen_A22B ]; then
    HIDDEN_SIZE=4096
    NUM_ATTN_HEADS=64
    NUM_LAYERS=94
...
if [ $DISPATCHER_TYPE = flex_deepep ]; then
    moe_options=" ${moe_options} --moe-token-dispatcher-type flex --moe-flex-dispatcher-backend deepep "
...
PREFIX="pretrain-qwen-${MODEL_SIZE}-bs-${BATCH_SIZE}-gbs-${GLOBAL_BATCH_SIZE}"
