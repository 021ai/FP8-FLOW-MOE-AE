...
case "${MODEL_CASE}" in
    deepseekv3)
        ...
        export CPT_PRETRAIN_CHECKPOINT_PATH=${CPT_PRETRAIN_CHECKPOINT_PATH:-/mnt/data/models/DeepSeek-V3-0324-bf16_torch_dist/}
        export TOKENIZER_TYPE=HuggingFaceTokenizer
        export TOKENIZER_PATH=${TOKENIZER_PATH:-/mnt/data/models/DeepSeek-V3-0324-bf16_torch_dist/release/}
        ...
        ENTRYPOINT="${SCRIPT_DIR}/pretrain_ds.sh"
        ;;
    deepseekv2)
        ...
        export CPT_PRETRAIN_CHECKPOINT_PATH=${CPT_PRETRAIN_CHECKPOINT_PATH:-/mnt/data/models/DeepSeek-V2}
        export TOKENIZER_TYPE=HuggingFaceTokenizer
        export TOKENIZER_PATH=${TOKENIZER_PATH:-/mnt/data/models/DeepSeek-V2/}
    ...
    qwen3)
        ...
        export CPT_PRETRAIN_CHECKPOINT_PATH=${CPT_PRETRAIN_CHECKPOINT_PATH:-/mnt/data/models/Qwen3-235B-A22B-Instruct-2507-mcore}
        export TOKENIZER_PATH=${TOKENIZER_PATH:-/mnt/data/models/Qwen3-235B-A22B-Instruct-2507-mcore}
        export TOKENIZER_TYPE=HuggingFaceTokenizer
        ...
        export TRAIN_TOKENS=278284541614
        export ENABLE_RAMPUP_BS=fix_lr
        ENTRYPOINT="${SCRIPT_DIR}/pretrain_qwen3.sh"
        ;;
    *)
...
if [[ "${MOCK_DATASET:-}" != "true" ]] && [[ ! -d "${TOKENIZER_PATH}" ]]; then
    echo "Error: TOKENIZER_PATH does not exist: ${TOKENIZER_PATH}" >&2
    exit 1
fi
if [[ ! -d "${CPT_PRETRAIN_CHECKPOINT_PATH}" ]]; then
    echo "Warning: CPT_PRETRAIN_CHECKPOINT_PATH does not exist: ${CPT_PRETRAIN_CHECKPOINT_PATH}" >&2
    echo "Training will start from random initialization." >&2
fi
