...
def test_main(num_sms: int, local_rank: int, num_local_ranks: int, num_ranks: int, num_nodes: int, rank: int, buffer: deep_ep.Buffer, group: dist.ProcessGroup):
    ...
    for config in configs:
        ...
        output['ModelSize'].append(f'{model_size}-{num_tokens}-{hidden}-{num_topk}-{num_experts}-{num_topk_groups}')
                    ...
                    for with_topk in (True, ):
                        ...
                        if local_rank == 0:
                            if isinstance(current_x, tuple):
                                output['FP8'].append(float(f'{t:.6f}'))
                            else:
                                output['BF16'].append(float(f'{t:.6f}'))
                            # print(f'[testing] Running with {"FP8" if isinstance(current_x, tuple) else "BF16"}, time={output}', flush=True, end='')
    ...
    if local_rank == 0:
        with open(f'internode-{num_nodes}-{num_ranks}-{local_rank}-{os.getpid()}.json', 'w', encoding='utf-8') as f:
